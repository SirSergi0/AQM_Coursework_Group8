...
\end{abstract}
\maketitle
\textcolor{red}{Sergio: I didn't know if you wanted to keep the hole templete, I leave after this section}
\section{\label{sec:MathematicalBackrground}Mathematical backrground of the algorithm}
% sections are not used for PRL papers

The goal of the algorithm is the following: given a positive integer $n$, find $m, r \in \mathbb{N}$, where $m,r>1$, such that $n = m r$, that is, to decompose $n$ as a product of two non-trivial positive integers.

Let $\tilde{m}$ be a candidate divisor of $n$, chosen such that $1 < \tilde{m} < n$. Using the Euclidean algorithm, one can compute $\gcd(\tilde{m}, n)$ in polynomial time.  

If $\gcd(\tilde{m}, n) \neq 1$, then we have found a non-trivial divisor of $n$. In this case, set $m = \gcd(\tilde{m}, n)$ and $r = \frac{n}{\gcd(\tilde{m}, n)}$.
...
This order $r$ always exists because $\left(\tfrac{\mathbb{Z}}{n\mathbb{Z}}\right)^*$ is a finite abelian group. In virtue of the Chinese remainder theorem we know $\left(\tfrac{\mathbb{Z}}{n\mathbb{Z}}\right)^*$ to be isomorphic to a direct product of cyclic groups. The efficient computation of $r$ is precisely the step where the quantum part of the algorithm becomes essential.  
